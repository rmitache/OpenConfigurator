...
(function (factory) {
    ...
    validation.rules['minObs'] = {
        validator: function (val, minObs) {
            return utils.isEmptyVal(val) || val >= minObs();
        },
        message: 'Please enter a value greater than or equal to {0}.'
    };
    validation.rules['maxObs'] = {
        validator: function (val, maxObs) {
            return utils.isEmptyVal(val) || val <= maxObs();
        },
        message: 'Please enter a value less than or equal to {0}.'
    };
    ...
    ko.bindingHandlers['validationElement'] = {
        update: function (element, valueAccessor) {
            ...
            var cssSettingsAccessor = function () {
                ...
                var shouldShow = isValid ? false : true; // (isModified ? !isValid : false); // Modified by Radu to always show invalid fields, even if they haven't been modified
    ...
    ko.extenders['validatable'] = function (observable, enable) {
        if (enable && !utils.isValidatable(observable)) {
            ...
            observable._disposeValidation = function () {
                ...
                if (observable.isModified._subscriptions)
                    observable.isModified._subscriptions['change'] = [];
                if (observable.isValidating._subscriptions)
                    observable.isValidating._subscriptions['change'] = [];
                if (observable.__valid__._subscriptions)
